import { Box3, Frustum, Matrix4, Vector3, Plane } from 'three';
import type { PlatformDetectionResult, UserPlatform } from './types.js';
 ...
 */
const checkOculusBrowser = (): PlatformDetectionResult => {
 ...
 * @returns an object indicating whether there is a match and the associated confidence level.
 */
const checkWolvicBrowser = (): PlatformDetectionResult => {
  ...
  const featureDetect = ((window as any).mozInnerScreenX != null && window.speechSynthesis == null);
 ...
 * @returns an object indicating whether there is a match and the associated confidence level.
 */
 const checkPicoBrowser = async (): Promise<PlatformDetectionResult> => {
 ...
 * @returns an object indicating whether there is a match and the associated confidence level.
 */
 const checkDesktopBrowser = (): PlatformDetectionResult => {
  // We are doing a coarse check here for lack of touch-capability and no Android/Mobile string in the UA.
  const featureDetect = (navigator.maxTouchPoints === 0 || (navigator as any).msMaxTouchPoints === 0);
...
const checkUserPlatform = async (): Promise<UserPlatform> => {
  if (checkOculusBrowser().match) {
    return { platform: 'Oculus', confidence: checkOculusBrowser().confidence };
  } else if (checkWolvicBrowser().match) {
    return { platform: 'Wolvic', confidence: checkWolvicBrowser().confidence };
  } else if ((await checkPicoBrowser()).match) {
    return { platform: 'Pico', confidence: (await checkPicoBrowser()).confidence };
  } else if (checkDesktopBrowser().match) {
    return { platform: 'Desktop', confidence: checkDesktopBrowser().confidence };
  } else {
...
const urlContainsUTMParams = (url: string): boolean => {
  return url.indexOf('utm_source=') !== -1 || url.indexOf('utm_campaign=') !== -1 || url.indexOf('utm_channel=') !== -1;
}

const appendUTMParams = (url: string, adUnitId: string): string => {
 ...
 */
const visibilityCheck = (bbMin: number[], bbMax: number[], cameraProjMatrix: number[], cameraWorldMatrix: number[]): boolean => {
  ...
  frustum.planes.forEach((plane: Plane) => plane.applyMatrix4(new Matrix4().fromArray(cameraWorldMatrix)));
